...
class SpotifyClient:
        ...
        try:
            ...
            if exc.http_status == 429:
                ...
                try:
                    ...
                    logger.warning("Retry failed: %s", retry_exc)
                    return []
            else:
                return []
    ...
    def create_playlist(
        ...
        """Create a playlist and add track URIs in batches of 100."""
        try:
            user = self._sp.current_user()
        except SpotifyException as exc:
            raise RuntimeError(f"Spotify authentication failed: {exc}") from exc
        user_id = str((user or {}).get("id") or "")
        if not user_id:
            raise RuntimeError("Could not read the current Spotify user id")
        try:
            ...
            raise RuntimeError(f"Could not create playlist: {exc}") from exc
        ...
        for i in range(0, len(uris), BATCH_SIZE):
            ...
            try:
                ...
                raise RuntimeError(f"Could not add tracks to playlist: {exc}") from exc
        logger.info("Created playlist %s with %s tracks", name, len(uris))
